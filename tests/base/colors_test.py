...
def test_Color_rgb_normalizado():
    assert colors.Color(Red=111, Green=222, Blue=25, size=8).rgb_normalized == (111 / 255, 222 / 255, 25 / 255)


def test_Color_rgba():
    assert colors.Color(Red=111, Green=222, Blue=25, Alpha=10).rgba == (111, 222, 25, 10)
...
def test_from_tuple():
    assert colors.from_tuple((10, 11, 12), 13, 8) == colors.Color(Red=10, Green=11, Blue=12, Alpha=13, size=8)
...
def test_random_color_from_base(rgb, max_dist):
    ...
    assert np.minimum(0, expected_r - max_dist) <= out_r <= np.minimum(base_color._max_value, expected_r + max_dist)
    assert np.minimum(0, expected_g - max_dist) <= out_g <= np.minimum(base_color._max_value, expected_g + max_dist)
    assert np.minimum(0, expected_b - max_dist) <= out_b <= np.minimum(base_color._max_value, expected_b + max_dist)
