...
def test_slice_image(shape, tmpdir):
    with create.ImageMaskFiles(
        str(tmpdir), shape[0], shape[1], ['example'], create_image=False,
    ) as paths:
...
def test_image_with_annotation(shape, annotations_ex, cluster_ex, tmpdir):
    with create.ImageMaskFiles(
        ...
        # test stop when take all annotations, and slice annotations into
        # others (if a annotation intersects the new size)
        sliced_quantity, annotations_items = slice.image_with_annotation(
            img_path, tmp_dir, annotations_ex, 100, 100,
        )
        ...
        # test stops when finish the slices, and copy the annotation (if a
        # annotation is completely inside of the new size)
        ann_items = annotations_ex[:]
        ann_items.append(
            Annotation(affinity.translate(cluster_ex, -9999, -9999), 2),
        )
        sliced_quantity, annotations_items = slice.image_with_annotation(
            img_path, tmp_dir, ann_items, 1, 2,
        )
...
@pytest.mark.slow
def test_single_core_image_and_annotations(
    shape, ccagt_df_single_nucleus, tmpdir,
):
    ...
    with create.ImageMaskFiles(
        ...
        outputs = slice.single_core_image_and_annotations(
            imgs,
        ...
        sliced_quantity, annotations_items = outputs
        assert sliced_quantity == 1
...
@pytest.mark.slow
def test_slice_images_and_annotations(
    shape, ccagt_multi_image_names, ccagt_ann_multi_path, tmpdir,
):
    with create.ImageMaskFiles(
        ...
        assert all(
            len(os.listdir(os.path.join(tmp_dir, 'images', s))) == qtd
            for s, qtd in slides_counter.items()
        )
