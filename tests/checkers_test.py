...
def test_single_core_mask_has(shape, tmpdir):
    with create.ImageMaskFiles(
        str(tmpdir), shape[0], shape[1], ['test'], create_image=False,
    ) as paths:
        _, mask_dir, _ = paths
        o = checkers.single_core_mask_has(
            {os.path.join(mask_dir, 'test.png')}, {0},
        )
        assert len(o) == 0

        o = checkers.single_core_mask_has(
            {os.path.join(mask_dir, 'test.png')}, {0, 1},
        )
        assert len(o) == 1
        assert o == {'test'}
...
@pytest.mark.slow
def test_masks_that_has(shape, tmpdir):
    with create.ImageMaskFiles(
        str(tmpdir), shape[0], shape[1], ['test'], create_image=False,
    ) as paths:
        _, mask_dir, _ = paths
