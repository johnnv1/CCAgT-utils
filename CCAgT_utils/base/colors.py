...
class Color(NamedTuple):
    ...
    def _max_value(self) -> int:
        ...
        _error_suf = (
            'value exceeds the max value for the color size! '
            f'(bits size={self.size} -> max_value={max_value})'
        )
...
# noqa: E501 - Based on https://github.com/cocodataset/panopticapi/blob/7bb4655548f98f3fedc07bf37e9040a992b054b0/panopticapi/utils.py#L43
def random_color_from_base(base: Color, max_dist: int = 30) -> Color:
    base_rgb = base.rgb
    new_color = base_rgb + np.random.randint(
        low=-max_dist,
        high=max_dist + 1, size=3,
    )
